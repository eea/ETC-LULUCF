...
import rasterio
import pandas as pd
from src.SOC_scenarios.utils.soc_helper_functions import resample_raster_to_ref
...
def _get_stats_window(window, settings, df_stratification):
    ...
    for i, strata in df_stratification.iterrows():
        ...
        for dataset in settings.get('DATASETS'):
            ...
            if dataset == 'SOC':
                continue
            nodatavalue = rasterio.open(settings.get('DATASETS').get(dataset)).profile.get('nodata')
            if nodatavalue is None:
                nodatavalue = 0
            values_dataset = rasterio.open(settings.get('DATASETS').get(dataset)).read(1, window=window)
            if np.all([values_dataset == nodatavalue]) and idx_dataset == 0:
                log.warning(f'NO STRATIFICATION COULD BE DONE FOR WINDOW {window} DUE TO NO VALID VALUES')
                return
        ...
        if lst_loc_strata:
            nodata_SOC = rasterio.open(settings.get('DATASETS').get('SOC')).profile.get('nodata')
            values_SOC = rasterio.open(settings.get('DATASETS').get('SOC')).read(1, window=window)
            if np.all([values_SOC == nodata_SOC]):
                continue

            # create an empty array to define the locations where all the conditions match based
...
def main_SOC_analysis(settings):
    ...
    if not os.path.exists(os.path.join(settings.get('outfolder'), 'stratification', outname))  \
        ...
        SLOPE_RANGE = [settings.get('Slope_classes').get(item) for item in SLOPE_CAT]

        #2 Environmental zones:
        ENV_CAT = list(settings.get('Env_zones_mapping').keys())
        ENV_RANGE = [settings.get('Env_zones_mapping').get(item) for item in ENV_CAT]
        # Translation table env classes

        #3 IPCC LULUCF classes based on CLC
        LUC_CAT = list(settings.get('CLC_cross_walk').get(f'LEVEL{str(Level_LUC)}').keys())
        LUC_RANGE = [settings.get('CLC_cross_walk').get(f'LEVEL{str(Level_LUC)}').get(item) for item in LUC_CAT]
        ...
        All_combinations_RANGE = list(product(SLOPE_RANGE, ENV_RANGE, LUC_RANGE))
        ...
        df_all_range = create_df_strata(All_combinations_RANGE, ['SLOPE_RANGE', 'ENV_RANGE', 'LUC_RANGE'])
        df_all_cat = create_df_strata(All_combinations_CAT, ['SLOPE_CAT', 'ENV_CAT', 'LUC_CAT'])

        df_full_stratification = pd.merge(df_all_cat, df_all_range, on='STRATA_ID')
        df_full_stratification['LEVEL_LUC'] = [Level_LUC] * df_full_stratification.shape[0]
        df_full_stratification.to_csv(os.path.join(settings.get('outfolder'), 'stratification', outname), index=False)


    # Open the stratification file needed for extracting the values per window
    df_full_stratification = pd.read_csv(os.path.join(settings.get('outfolder'), 'stratification', outname))
    ...
    for dataset_name in settings.get('DATASETS'):
        ...
        if ix == 0:
            ...
            xdim, ydim = rasterio.open(settings.get('DATASETS').get(dataset_name)).shape
        else:
            if rasterio.open(settings.get('DATASETS').get(dataset_name)).shape != (xdim, ydim):
                raise ValueError(f'Dimensions of datasets do not match {dataset_name}')
...
if __name__ == '__main__':
    ...
    # CLC IPCC mapping refinement contains the cross-walk between CLC and IPCC at two defined levels
