...
def define_affor_areas(settings, slope_max = 87.5):
    ...
    if not os.path.exists(outdir_mask.joinpath(outname_mask)) or overwrite:
        ...
        for LUCAT in factor_scenario.keys():
            ...
            mask_raster[((External_mask_raster == 1)& (CLC_IPCC_LUCAT_raster == value_LUCAT) & (N2K_raster == 255))] = 1  # 1 is suitable
...
def create_affor_potential(settings, affor_mask_array):
    ...
    if not os.path.exists(affor_pot_dir) or settings.get('overwrite'):
        ...
        if settings.get('block_based_processing'):
            ...
            CLC_IPCC_climate_raster, src_IPCC_climate = open_raster_from_window\
                                                        (settings.get('path_IPCC_climate_resampled'), bounds)

            if CLC_IPCC_climate_raster.size == 0:
                logger.info(f'NO PIXEL DATA AVAILABLE FOR NUTS REGION: {settings.get("NUTS3_info").NUTS_ID}')
                return None
            no_data_IPCC_climate = src_IPCC_climate.get('nodata')

            stats = zonal_stats(settings.get('NUTS3_info').geometry, CLC_IPCC_climate_raster, affine=src_IPCC_climate.get('transform'),
                                nodata=no_data_IPCC_climate, stats='majority')
            climate_ID = stats[0].get('majority')
            if climate_ID == 0:
                stats = zonal_stats(settings.get('NUTS3_info').geometry, CLC_IPCC_climate_raster, affine=src_IPCC_climate.get('transform'),
                                    nodata=no_data_IPCC_climate, stats='max')
                climate_ID = stats[0].get('max')



            ## get the ecological zone
            shp_ecozone = gpd.read_file(settings.get('path_ecological_zones'))
            ## now derive in which eco zone we are #TODO derive it pixel-based not NUTS-based
            ecozone = [item for item in shp_ecozone.iterrows() if item[1].geometry.intersects(settings.get('NUTS3_info')
                                                                                              .geometry)][0][1]['GEZ_TERM']
            ...
            for LUCAT in factor_scenario.keys():
                ...
                if tree_species in df_trees_biom_increment.Tree_species.to_list():
                    ...
                    ann_increment_tree_species = df_trees_biom_increment.loc[df_trees_biom_increment
                                                                                 .Tree_species == tree_species]['Ann_increment_min'].values[0]
                    ## get the value assigned to the specific IPCC LUCAT
                    value_LUCAT = df_factor_IPCC_factors.loc[df_factor_IPCC_factors['IPCC_landuse_name'] == LUCAT] \
                        ['IPCC_landuse_id'].values[0]
                    ...
                    kg_to_ton = 1000
                    """
                    #divide by 1000 to convert to tonC/ha*yr

                    ## RTS factor
                    try:
                        RTS_value_year1 = max(df_RTS_factors_trees.loc[((df_RTS_factors_trees['Ecological zone'] == ecozone)
                                                                  & (df_RTS_factors_trees['Tree_species']
                                                                     == tree_species))]['RTS'])
                    except:
                        continue

                    ## forest type:
                    forest_type = df_forest_type_info.loc[df_forest_type_info['Tree species']
                                                          == tree_species]['Forest type'].values[0]


                    ## determine when the RTS value should be adjusted
                    thr_AGB_RTS  = int(df_RTS_factors_trees.loc[((df_RTS_factors_trees['Ecological zone'] == ecozone)
                                                             & (df_RTS_factors_trees['Tree_species']
                                                                == tree_species))]['AGB size'].values[0][1:])

                    ## determine the different periods for which the BCEF factor should be adjusted
                    ages_BCEF_adjust = list(df_BCEF_factors_trees.loc[((df_BCEF_factors_trees['IPCC_climate_id'] == climate_ID)
                                                                       & (df_BCEF_factors_trees['Forest type']
                                                                          == forest_type))]['Age'].values)
                    ages_BCEF_adjust = [item.split('-')[0] for item in ages_BCEF_adjust]

                    ## count the amount of years to reach this threshold
                    nr_years_pred = settings.get('year_potential') - settings.get('year_baseline')
                    tot_biomass = 0
                    age = 0
                    for idx_year in range(nr_years_pred):
                        if tot_biomass < thr_AGB_RTS:
                            RTS_factor = RTS_value_year1
                        else:
                            RTS_factor = min(df_RTS_factors_trees.loc[((df_RTS_factors_trees['Ecological zone'] == ecozone)
                                                          & (df_RTS_factors_trees['Tree_species']
                                                             == tree_species))]['RTS'])

                        age_BCEF_select = [int(item) for item in ages_BCEF_adjust if age >= int(item)][-1]
                        BCEF_factor = df_BCEF_factors_trees.loc[((df_BCEF_factors_trees['IPCC_climate_id']
                                                                  == climate_ID) & (df_BCEF_factors_trees['Forest type']
                                                                                    == forest_type)
                                                                 & df_BCEF_factors_trees['Age'].str.contains(str(age_BCEF_select) + '-'))]['BCEF'].values[0]

                        yrly_increase = ann_increment_tree_species * BCEF_factor * RTS_factor * 0.47 * A_pixel_ha
                        tot_biomass += yrly_increase

                        age += 1

                    ## 44/12 is the conversion factor for going from C to CO2
                    avg_yrly_increment = int((tot_biomass/nr_years_pred) * 44/12 * scaling)

                    affor_yrly_pot_raster[loc_affor] = avg_yrly_increment
...
def calc_stats_biomass_NUTS(raster_dir: str, spatial_layer: gpd,
    ...
    for FLU_class in df_FLU_mapping['IPCC_landuse_id'].unique():
        ...
        ## determine the threshold at which the RTS should be adjusted due to an AGB value
        if settings.get('run_NUTS_specific_scenario'):  #otherwise use default factors
            factor_scenario_tree_species = get_factors_from_NUTS(settings, settings.get('factor_scenario'), 'Tree_species'
                                                                 , id_LUT_carbon_pool='afforestation')
            tree_species = factor_scenario_tree_species.get(IPCC_cat).get('Tree_species')

        else:
            tree_species = settings.get('afforestation_scenario').get(IPCC_cat).get('Tree_species')

        #TODO allow to adjust for high AGB values
        try:
            RTS_ratio = max(df_RTS_factors_trees.loc[((df_RTS_factors_trees['Ecological zone'] == ecozone)
                                                            & (df_RTS_factors_trees['Tree_species']
                                                               == tree_species))]['RTS'])
        except:
            RTS_ratio = np.nan
